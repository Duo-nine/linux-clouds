...
void* tcpClientRecvMessage(void *arg);
...
int main(int argc, char **argv)
{
	...
	if( -1 == (server_socket = socket(AF_INET, SOCK_STREAM, 0)) )//tcp协议使用SOCK_STREAM流式套接字
	{
		perror("socket");
		return -1;
	}
	...
	while(1)
	{
		...
		tmp_fds = fds;	//拷贝一份集合到tmp_fds专门用于select检查
		...
		{
			/* 发现监听套接字被保留在检查后的集合中 说明需要接收客户端的连接 */
			...
			{	
				...
				//创建线程,将tcp客户端处理函数传进来
				pthread_create(&pt, NULL, tcpClientRecvMessage, NULL);
			}
			else
			{
